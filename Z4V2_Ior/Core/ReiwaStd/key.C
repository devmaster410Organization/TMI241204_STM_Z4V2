...
{K_PONPOFF | K_ON},{K_MODE | K_ON},{K_TIMRESET | K_ON},
};
...
static uint16_t scan_port( void )
{
	...
	if( PORT_READ(BT1_IN) == GPIO_PIN_RESET ){ // 
		uw |= K_PONPOFF;
	}
	if( PORT_READ(BT2_IN) == GPIO_PIN_RESET ){ // 
		uw |= K_MODE;
	}
	if( PORT_READ(BT3_IN) == GPIO_PIN_RESET ){ // 
		uw |= K_TIMRESET;
	}
	return uw;
