...
#include <ctype.h>
#include "prj.h"
extern USBD_HandleTypeDef hUsbDeviceFS;	//フルスピード
...
uint16_t usbGetChar( uint32_t timeout )
{
   ...
   if( USB_RCV_TIMEOUT_CODE == usbcb.ungetdata ){
		osEvent event = osMessageGet(usbRcvQueue01Handle, timeout);  //timout  1ms
		switch( event.status ){
		case osEventMessage:
			data = event.value.v;
			break;
...
static int cmd_power( void )
{
	...
	scanf_result = sscanf(usbcb.word_top_ptr[1], "%d ", &i32);
	if(  (scanf_result == 1 ) &&  (i32 >= 1) && (i32 <= pModel->rel_max) ){
		RLMAN_set(i32-1,RLCAUSE_CMD);	//コマンドによりセット
		tSys.setup_update = 1; // indicate setup update
		snprintf(str,sizeof(str), "relay no = :%d",i32);
		usb_puts(str);
	}else{
...
static int cmd_status(  void )
{
	...
	if( usbcb.word_num == 1 ){ // status command
		usb_puts("Status:");
		snprintf(str,sizeof(str), "Model:%s",tModel[tSys.model].model_name);
		usb_puts(str);
		snprintf(str,sizeof(str), "dsw:0x%02X", Get_DipSw());
		usb_puts(str);
		snprintf(str,sizeof(str), "BAT:%5.2f V", Get_vbat());
		usb_puts(str);
		snprintf(str,sizeof(str), "EX Board :%d,%s", tSys.connect_board_cnt ,tPS_SEL[PORT_READ(PS)==0 ? 0 : 1]);
		usb_puts(str);
		snprintf(str,sizeof(str), "Xport:%d", !tSys.isWiznet);
		usb_puts(str);
	}
	return 0;
